...
int validmove(char *piece, char *move){
    ...
    if(board[X(piece)][Y(piece)] == 'b'){
        ...
        if(move[0] == (piece[0]+2) && move[1] == (piece[1]+2)){
            if(toupper(board[X(piece)+1][Y(piece)+1]) != 'W'){
...
struct sockaddr_in createUDPSockclient(int *server_fd){
    //0. Variables
    ...
    if (((*server_fd) = socket(AF_INET, SOCK_DGRAM, 0)) < 0){
        perror("1. socket not created (FAIL)\n");
        exit(EXIT_FAILURE);
    }
    flushprintf("||           Socket created                          ",HALF_SEC);

    //2. Defining the address of the server (same server.c)
    memset(&address, 0, sizeof(address));
    address.sin_family = AF_INET;
    address.sin_addr.s_addr = INADDR_ANY;
    address.sin_port = htons(PORT);
    flushprintf("Address Defined                  ||\n",HALF_SEC);
    printf("|| Conected !!!                                                                       ||\n");
    printf("========================================================================================\n");
    //2. Returning server fd
    return address;
}
...
int main(int argc, char const *argv[]){
    ...
    socklen_t address_s_len = sizeof(address_s);

    //1. Entering in game server
    welcome();
    address_s = createUDPSockclient(&server_fd);

    //2. Informing the server that you are online
    //sendto server the message with len and wait the confirmation of address with this size
    sent_flag = sendto(server_fd , ONLINE_MSG , strlen(ONLINE_MSG) , MSG_CONFIRM,(const struct sockaddr *)&address_s,sizeof(address_s));
    if(sent_flag < 0){
    ...
    //3. Receiving server player name
    //recvfrom server a message and store in buffer with size BUFFER_SIZE and wait all dependencies from address with len
    if((msg_flag = recvfrom(server_fd,opponent,NAME_SIZE,MSG_WAITALL,(struct sockaddr *)&address_s,&address_s_len)) < 0){
        perror("listen");
        exit(EXIT_FAILURE);
    }

    //4. Informing the server that you are ready
    sent_flag = sendto(server_fd , player , NAME_SIZE , MSG_CONFIRM,(const struct sockaddr *)&address_s,sizeof(address_s));
    if(sent_flag < 0){
        printf("> There was an error... shutting down the system.\n");
        return 0;
    }
    ...
    while(!winner){
        ...
        while(match((char *)op_turn,"^[Nn][Oo]*$")){
            ...
            if(turn == 0){
                ...
                if((msg_flag = recvfrom(server_fd, (void *)piece,PIECE_SIZE,MSG_WAITALL,(struct sockaddr *)&address_s,&address_s_len)) < 0){
                    perror("> There was an error in recvfrom... shutting down the system.");
                    exit(EXIT_FAILURE);
                }
                //ii. receiving the move
                if((msg_flag = recvfrom(server_fd, (void *)move,MOVE_SIZE,MSG_WAITALL,(struct sockaddr *)&address_s,&address_s_len)) < 0){
                    perror("> There was an error in recvfrom... shutting down the system.");
                    exit(EXIT_FAILURE);
                }
            ...
            if((msg_flag = recvfrom(server_fd, (void *)op_turn,10,MSG_WAITALL,(struct sockaddr *)&address_s,&address_s_len)) < 0){
                perror("> There was an error in recvfrom... shutting down the system.");
                exit(EXIT_FAILURE);
            }
            piece[0] = move[0];
        ...
        while(match((char *)pl_turn,"^[Nn][Oo]*$")){
            if(turn == 0){
                //i. reading the play
                printf("Your turn! Choose the piece (BLACK) and the movement.\n");
                printf("> Write the piece and the move coordinate, respectively (letter-number)\n");
                printf("> Exemple: D1 E2\n");
                while(strcmp(piece,"") == 0 && strcmp(move,"") == 0){
                    ...
                    else if(toupper(board[toupper(piece[0])-'A'][piece[1]-'0']) != 'B'){
                ...
                sent_flag = sendto(server_fd ,piece,PIECE_SIZE,MSG_CONFIRM,(const struct sockaddr *)&address_s,address_s_len);
                if(sent_flag < 0){
                    printf("> There was an error... shutting down the system.\n");
                    return 0;
                }
                sent_flag = sendto(server_fd ,move,MOVE_SIZE,MSG_CONFIRM,(const struct sockaddr *)&address_s,address_s_len);
                if(sent_flag < 0){
                    printf("> There was an error... shutting down the system.\n");
                    return 0;
                }
                ...
                //iii. sendint the information to the opponent
                sent_flag = sendto(server_fd ,move,MOVE_SIZE,MSG_CONFIRM,(const struct sockaddr *)&address_s,address_s_len);
                if(sent_flag < 0){
                    printf("> There was an error... shutting down the system.\n");
                    return 0;
            ...
            sent_flag = sendto(server_fd ,pl_turn,10,MSG_CONFIRM,(const struct sockaddr *)&address_s,address_s_len);
            if(sent_flag < 0){
                printf("> There was an error... shutting down the system.\n");
                return 0;
            }
