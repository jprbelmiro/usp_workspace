...
void norma1( double **matrix , int c, int r){
// Responsável por calcular a norma de uma matriz...
    int i, j;
...
// Projeto : Norma 1
    ...
    //Lendo as matrizes do arquivo e calculando sua norma1...
    mtz *file1 = malloc(sizeof(mtz));
        readamatrix(file1,matrix1_file);
            norma1(file1->matrix,file1->size,file1->size);
    mtz *file2 = malloc(sizeof(mtz));
        readamatrix(file2,matrix2_file);
            norma1(file2->matrix,file2->size,file2->size);

    //Calculando a soma das duas matrizes, imprimindo a matriz resultado e sua norma na tela...
    double ** matrixresult = matrixsum(file1->matrix,file2->matrix,file1->size,file1->size);
    printMat(matrixresult,file1->size,file1->size);
        norma1(matrixresult,file1->size,file1->size);
